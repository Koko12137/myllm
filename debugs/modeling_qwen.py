...
class Qwen2Attention(nn.Module):
    ...
    def __init__(self, config: Qwen2Config, layer_idx: int):
        ...
        self.head_dim = getattr(config, "attention_head_dim", config.hidden_size // config.num_attention_heads)
...
class Qwen2DecoderLayer(nn.Module):
    def __init__(self, config: Qwen2Config, layer_idx: int):
        ...
        # Step 5: Replace RMSNorm
        self.input_layernorm = MyLLMRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = MyLLMRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def __init__(self, config: Qwen2Config):
        ...
        self.norm = MyLLMRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = self.rotary_emb = MyLLMRotaryEmbedding(self.head_dim, config.max_position_embeddings, config.rope_theta)
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        # Step 3: Replace the causal mask
        ...
        # Step 4: Replace the position embeddings
        # Create a ones-like states with shape of (1, 1, seq_len, attention_head_dim)
        position_embeddings = torch.ones(1, 1, seq_len, self.head_dim, device=hidden_states.device)
    ...
    def _prepare_4d_causal_mask(
        ...
        else:   # Step 3: Replace the causal mask
...
class Qwen2ForCausalLM(Qwen2PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.loss_fn = nn.CrossEntropyLoss()    # Step 1: Replace the loss function
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        logits = logits.view(-1, self.vocab_size)   # Step 1: Flatten the hidden states
        
        # Mask the logits and labels, we do not update the padding tokens
        logits = logits[attention_mask.view(-1)]            # Step 2: Only keep the logits for the non-masked tokens
        labels = labels.view(-1)[attention_mask.view(-1)]   # Step 2: Only keep the labels for the non-masked tokens

        loss = None
        if labels is not None:
            loss = self.loss_fn(logits, labels)   # Step 1: Calculate the loss
